  ...
  void move(int frameRate, int elevatorAlgorithm) { 
   ...
   if(elevatorAlgorithm == 0) {
      ...
      */
      ...
      //determine currentDestination
      //-------------------------------------------------------------------------
      //if there is at least one job in the elevators service queue
      if(serviceQueue.size() >= 1) {
        
        
        //if the job has already been picked up
        ...
        } else { //the job has not been picked up
      ...
      if(serviceQueue.size() >= 1) {
        //call the appropriate movement function OR set the elevator to STATIONARY in order to indicate door pickup
        if(currentDestination > floor) {
          direction = Direction.UP;
          moveUp();
        
        } else if (currentDestination == floor & !doneOnFloor) {
          
          
          //if the door is not closing, but is also not opening
          if(!doorClosing & !doorOpening) {
            
            openDoor(frameRate);
            
            
          } else if (doorOpening) { //if the door is currently open
          ...
          } else if (doorClosing) { //if the door is currently closing 
            boolean closingAnimationDone = false; 
            closingAnimationDone = doorClosingTimer.tickTime();
            ...
            if(closingAnimationDone) {
              ...
              doneOnFloor = true;
              needInstruction = true;
              System.out.println("done with door stuff for this floor");
          ...
          //the elevator should only ever pick up passengers when it is moving downwards
          if(specialDoorStop) {
            System.out.println("EXTRA pickup needed !"); 
      ...
      if(serviceQueue.size() >= 1) {
        ...
        if(currentDestination > floor) {
          ...
          //if the door is not closing, but is also not opening, or if the elevator just dropped a passenger
          if(!doorClosing & !doorOpening) {
      ...
      if(serviceQueue.size() >= 1) {
        if(direction == Direction.STATIONARY) {
          System.out.println("Stationary elevator");
