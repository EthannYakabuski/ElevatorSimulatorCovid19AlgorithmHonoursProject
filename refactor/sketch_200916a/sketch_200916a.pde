...
void draw() {
  ...
  if(simulationStarted) {
    ...
    if (elevatorAlgorithm == 2 || elevatorAlgorithm == 1) { distributeStandardTasks(); }
    
    if (elevatorAlgorithm == 2 || elevatorAlgorithm == 1) { checkSpecialPickups(); }
    
    if (elevatorAlgorithm == 2 || elevatorAlgorithm == 1) { checkElevator2ExtraPickups(); }
    
    if (elevatorAlgorithm == 2 || elevatorAlgorithm == 1) { checkStuckElevators(); }
    
    if (elevatorAlgorithm == 2 || elevatorAlgorithm == 1) { 
      
      //fixes the visual display glitch
      for(int i = 0; i < elevators.size(); i++) {
        if(!elevators.get(i).doorOpening & !elevators.get(i).doorClosing) {
          elevators.get(i).cabPosX = elevators.get(i).originalCabPosX; 
  ...
  //  if (elevatorAlgorithm == 2) { checkStuckElevators(); }
   ...
   //if(buildingObjectsCreated) { testDelayedPassengerRequests(); }
    ...
    if(!simulationStarted) {
      ...
      if(highRise.peopleStatistics.size() > 2) {
...
void checkSpecialPickups() {
  ...
  for(int i = 0; i < elevators.size(); i++) {
    ...
    if(thereIsASpecialPickup & !elevators.get(i).doorClosing & !elevators.get(i).doorOpening & !elevators.get(i).doneOnFloor) {
...
void outputAllStatistics() {
  ...
  if(!outputStats) {
    int peopleStatSize = highRise.getPeopleStats().size(); 
  
    System.out.println("There are a total of " + peopleStatSize + " people statistics to sift through"); 
  
    int totalFramesWaited = 0; 
  
    String[] peopleStats = new String[highRise.getPeopleStats().size()];
  
    //for each people stat in the building
    for(int i = 0; i < highRise.getPeopleStats().size(); i++) {
    
      peopleStats[i] = highRise.getPeopleStats().get(i).makeString(); 
    
    }
  
    saveStrings("data/peopleStatistics.txt",peopleStats); 
  
    String[] elevatorStats = new String[elevators.size()];
  
    //for each elevator, output their information
    for(int i = 0; i < elevators.size(); i++) {
    
      elevatorStats[i] = elevators.get(i).getElevatorStat().makeString();
    }
    
    saveStrings("data/elevatorStatistics.txt",elevatorStats);
...
void checkEndOfSimulation() {
  ...
  if(eventSchedulerLoaded) {
    ...
    boolean elevatorsDoneWork = false; 
    
    if(eventScheduler.getSimulationEvents().size() == 0) {
      
      //now check if every elevator is still
      
      int workTally = 0; 
      
      for(int  i = 0; i < elevators.size(); i++) {
        
        workTally = workTally + elevators.get(i).getServiceQueue().size(); 
        
        
      }
      
      if(workTally > 0) {
        elevatorsDoneWork = false; 
      } else {
        elevatorsDoneWork = true;  
      }
  ...
  for(int i = 0; i < elevators.size(); i++) {
    ...
    if(elevatorAlgorithm == 2) {
      ...
      if(elevators.get(i).getFloor() == 1) {
        ...
        if(elevators.get(i).getCabPassengers().size() >= 1) {
          ...
          elevators.get(i).floorFrames++; 
          
          if(elevators.get(i).floorFrames > 75) {
            
            for(int p = 0; p < elevators.get(i).getCabPassengers().size(); p++) {
              
              elevators.get(i).getElevatorStat().tickPassengerDrop();
              highRise.peopleStatistics.add(new PeopleStat(elevators.get(i).getCabPassengers().get(p).getFramesWaitedForElevator(),elevators.get(i).getCabPassengers().get(p).getFramesSpendOnElevator()));
            
            }
            elevators.get(i).floorFrames = 0; 
            elevators.get(i).getServiceQueue().clear();
            elevators.get(i).getCabPassengers().clear();
          }
    ...
    if(elevatorAlgorithm == 2) {
      ...
      if(elevators.get(i).getServiceQueue().size() == 1) { 
        ...
        if(elevators.get(i).openedDoorFloor) { 
         
          if (elevators.get(i).currentDestination == 1) { 
            ...
            if(elevators.get(i).getCabPassengers().size() == 1) {
              ...
              if(elevators.get(i).getFloor() > 1) { 
                ...
                if(elevators.get(i).direction == Direction.DOWN) {
                  
                  if(!elevators.get(i).doorClosing & !elevators.get(i).doorOpening) {
                    
                    //problem elevator
                    System.out.println("---------------------------------------------------------------------------------"); 
                    elevators.get(i).setDestination(elevators.get(i).getServiceQueue().get(0).getPickup());
                    //elevators.get(i).moveDown();
                    //elevators.get(i).floor = elevators.get(i).getFloor()-1;
    ...
    if(elevators.get(i).getFloor() == 1 & elevatorAlgorithm != 2) {
...
void checkMissedPickups() {
  ...
  for(int i = 0; i < elevators.size(); i++) {
    ...
    if(Direction.STATIONARY == elevators.get(i).getDirection()) {
...
void checkElevators() {
  ...
  for(int i = 0; i < elevators.size(); i++) {
    ...
    //for each elevator, check if needInstruction is set to true
    if(elevatorAlgorithm == 2) {
      
      if(elevators.get(i).needInstruction) {
        
        //need to pick up every person on this floor, door has already opened and closed
          ...
          for (int population = 0; population < highRise.getFloors().get(elevatorFloor).getRoomsList().get(room).getWhoIsHome().size(); population++) {
              ...
              if (!(highRise.getFloors().get(elevatorFloor).getRoomsList().get(room).getWhoIsHome().get(population).getJob().getID() == -1)) {
                ...
                if(pandemicStatus) {
                  ...
                  if(elevators.get(i).getCabPassengers().size() >= 1) {
                    allowedPickup = false;
                    //elevators.get(i).currentDestination = 1;
                  }
                 ...
                 System.out.println("adding a person to the cab " + i + " on the floor " + elevatorFloor); 
        ...
        for (int room = 0; room < highRise.getFloors().get(elevatorFloor).getRoomsList().size(); room++) {
          ...
          for (int population = 0; population < highRise.getFloors().get(elevatorFloor).getRoomsList().get(room).getWhoIsHome().size(); population++) {
              ...
              if (!(highRise.getFloors().get(elevatorFloor).getRoomsList().get(room).getWhoIsHome().get(population).getJob().getID() == -1)) {
                ...
                //this algorithm should only add people if there is not currently an person in the cab, 
                //this is modeling the scenario where the door opens but the person refuses the other person to get on
                //because they are in a pandemic one-family per cab scenario
    ...
    /*
    //in this case, it will check whether or not an elevator is currently on the floor of its job
    if(elevatorAlgorithm == 2) {
      
      //there will only ever be one, or none job with this style of algorithm
      if(elevators.get(i).getServiceQueue().size()>=1) {
        if(elevators.get(i).getFloor() == elevators.get(i).getServiceQueue().get(0).getPickup()) {
          
           if(!elevators.get(i).doorOpening & !elevators.get(i).doorClosing & !elevators.get(i).openedDoorFloor & elevators.get(i).getDirection() != Direction.DOWN) {
             System.out.println("setting stationary");
    ...
    //if the elevator algorithm is standard, and the elevator needs instruction, then you should pick up the person, anyone else on the same floor as well as
    //set the direction to down
    if((elevatorAlgorithm == 2) & (elevators.get(i).getNeedInstruction())) {
    ...
    if(elevators.get(i).getNeedInstruction() || elevators.get(i).getExtraPickupAble()) {
      ...
      if(elevators.get(i).serviceQueue.size() >= 1) {
        ...
        if(elevatorAlgorithm == 0) {  //covid-19 algorithm
          ...
          //update the elevators statistics
          elevators.get(i).getElevatorStat().tickPassengerPick(); 
...
//this function is responsible for resetting the bearings of an elevator
//that has stopped to open its doors BUT in the meantime another elevators has already finished
//closing its doors and picked up this passenger
...
void checkStuckElevators() {
  ...
  for(int i = 0; i < elevators.size(); i++) {
    ...
    if(elevators.get(i).direction == Direction.STATIONARY || elevators.get(i).direction == Direction.DOWN) {
      ...
      if( !elevators.get(i).doorOpening & !elevators.get(i).doorClosing ) {
        ...
        //if the elevator has no passengers (and isnt on floor 1)- just set destination to 1 and direction to down
        if(elevators.get(i).getFloor() > 1) {
          ...
          if(elevators.get(i).getCabPassengers().size() == 0) {
            if(elevators.get(i).getServiceQueue().size() >= 1) {
              System.out.println("DONT WORRY, I KNOW YOU ARE STUCK"); 
              elevators.get(i).getServiceQueue().remove(0); 
              elevators.get(i).direction = Direction.DOWN; 
              elevators.get(i).currentDestination = 1; 
            }
            
          } else {  //there are some passengers
             elevators.get(i).direction = Direction.DOWN; 
              elevators.get(i).currentDestination = 1;
            /*
            if(elevators.get(i).getServiceQueue().size() >= 1) {
              
              if(elevators.get(i).getServiceQueue().get(0).pickedUp) {
                 elevators.get(i).currentDestination = elevators.get(i).getServiceQueue().get(0).getDestination(); 
                 elevators.get(i).direction = Direction.DOWN; 
                 System.out.println("DONT WORRY, I KNOW YOU ARE STUCK, PASSENGER ON BOARD");
              } else {
                elevators.get(i).currentDestination = elevators.get(i).getServiceQueue().get(0).getPickup(); 
                 elevators.get(i).direction = Direction.DOWN; 
                 System.out.println("DONT WORRY, I KNOW YOU ARE STUCK, PASSENGER ON BOARD");
  ...
  if(elevatorAlgorithm == 1) { 
    //checking to see if there is an elevator stuck on the bottom floor
    for(int i = 0; i < elevators.size(); i++) {
      
      if ((elevators.get(i).getFloor() == 1) & (elevators.get(i).direction == Direction.STATIONARY) & (!elevators.get(i).doorClosing) & (!elevators.get(i).doorOpening) ) {
        ...
        if(elevators.get(i).getServiceQueue().size() >= 1) {
         ...
         if(elevators.get(i).getCabPassengers().size() == 0) {
           ...
           System.out.println("Algo 1 stuck"); 
           elevators.get(i).getServiceQueue().remove(0); 
...
String describeSimulationStart() {
  ...
  if(elevatorAlgorithm == 0) {
    ...
    returnString += "--- Using Standard algorithm in the elevators "; 
...
void drawElevatorTextInfo() {
  ...
  for(int i = 0; i < elevators.size(); i++) {
    ...
    text(elevators.get(i).getCabPassengers().size(), 755, 30+i*37);
    ...
    */
    ...
    if(elevators.get(i).getDirection() == Direction.STATIONARY) {
      text("still", 1050, 30+i*37);
      
    } else if (elevators.get(i).getDirection() == Direction.UP) {
      text("up", 1050, 30+i*37);
      
    } else if (elevators.get(i).getDirection() == Direction.DOWN) {
      text("down", 1050, 30+i*37);
      
    } else if (elevators.get(i).getDirection() == Direction.OPEN) {
      text("open", 1050, 30+i*37); 
    
    } else if (elevators.get(i).getDirection() == Direction.CLOSE) {
      text("close", 1050, 30+i*37);
